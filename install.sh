...
install_virtualmin_release () {
  ...
  if [ "$os_type" = "ubuntu" ]; then
    ...
    case "$os_version" in
      8*)
      run_ok "apt-get install --assume-yes apt-transport-http lsb-release ca-certificates" "Installing extra dependencies for Debian 8"
...
if [ $errors -eq "0" ]; then
  ...
  log_success "to configure at http://${hostname}:10000 (or http://${address}:10000)."
